...
declare function bibl2html:record($nodes) {
    ...
    return 
        ...
        if(bibl2html:responsibility($nodes) != '') then 
            if(ends-with(bibl2html:responsibility($nodes),'.')) then bibl2html:responsibility($nodes) else (normalize-space(string-join(bibl2html:responsibility($nodes),'')),'.') else (), 
...
declare function bibl2html:monograph($nodes as node()*) {
    ...
    return 
        (if(deep-equal($nodes/tei:editor | 
            ...
            if($nodes/tei:biblScope[@type='issue']) then
                ('.',$nodes/tei:biblScope[@type='issue'],' ')
            else (),
            if($nodes/tei:imprint/tei:date[text() != '']) then
               (' (',$nodes/tei:imprint/tei:date,')')
            else (),':',
            if($nodes/tei:biblScope[@type='pp']) then
                (' ',$nodes/tei:biblScope[@type='pp'])
            else (),
            '.'),'')))
    (:
    (if(deep-equal($nodes/tei:editor | 
