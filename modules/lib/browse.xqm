...
declare function browse:group-results($node as node(), $model as map(*), $collection as xs:string?){
    ...
    let $authors := distinct-values(
                        for $a in $hits//tei:author 
                        let $author := normalize-space($a) 
                        return $author)
    return 
        map {"group-by-authors" := 
                if(request:get-parameter('author-exact', '')) then 
                   ...
                   return 
                    ...
                    let $author-string := normalize-space(string-join($author,' '))
                    order by global:build-sort-string($author-string,'') 
                    return 
                        <author xmlns="http://www.w3.org/1999/xhtml" name="{$author-string}"/>
       }    
};