...
declare function data:get-records($collection as xs:string*, $element as xs:string?){
    ...
    return 
        ...
        (: Generic :)             
            ...
            for $hit in $hits
            let $root := $hit/ancestor-or-self::tei:TEI
            let $sortString := string-join($hit/text(),'')
            let $sort := normalize-space(global:build-sort-string($sortString,request:get-parameter('lang', '')))
            let $id := $root/descendant::tei:publicationStmt/tei:idno[1]
            group by $facet-grp := $id
        ...
        else if(request:get-parameter('view', '') = ('title')) then 
            for $hit in $hits
            let $root := $hit/ancestor-or-self::tei:TEI
            let $sortString := string-join($hit/text(),'')
            let $sort := normalize-space(global:build-sort-string($sortString,request:get-parameter('lang', '')))
            let $secondarySort := $root/descendant::tei:author[1]/descendant::tei:surname
            let $id := $root/descendant::tei:publicationStmt/tei:idno[1]
            group by $facet-grp := $id
            order by $sort[1],$secondarySort collation 'http://www.w3.org/2013/collation/UCA'
            where matches($sort,global:get-alpha-filter())
            return <browse xmlns="http://www.tei-c.org/ns/1.0" sort="{string($sort[1])}">{$root[1]}</browse>              
        else 
            for $hit in $hits
            let $root := $hit/ancestor-or-self::tei:TEI
            let $sort := normalize-space(global:build-sort-string($hit,request:get-parameter('lang', '')))
            (:let $id := $root/descendant::tei:publicationStmt/tei:idno[1]
              group by $facet-grp := $id:)
            let $secondarySort := global:build-sort-string($root/descendant::tei:title[1],())
            order by $sort, $secondarySort collation 'http://www.w3.org/2013/collation/UCA'
            where matches($sort,global:get-alpha-filter())
            return <browse xmlns="http://www.tei-c.org/ns/1.0" sort="{$sort}">{$root}</browse> 
(:
